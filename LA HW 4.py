...
#1
"""Is the number complex or real? 
   If the number is real then check if the value is less that 0. 
   If the value is lest than 0, multply by -1 and return the number.
   Else just return the number.

Args:
    a: It is an input value that puts a real number.
    b: It is an input value that puts a complex number.

Returns:
    If the number is complex, take the coeffiecients of the complex number 
    and square both numbers and add them together to get a square root.
"""

def abs_f(input_value):
    return_value = 0

    if isinstance(input_value, complex):
       # do something
       a = input_value.real**2
       b = input_value.imag**2
...
#2
"""A function that takes the argument and is set to default as 2.

Args:
    vector: It is stored as a list.
    inf: It is an integer valued scalar set to default at 2.

Returns:
    It returns the norm of the input vector.
"""
def inf_norm(vector: list[float], inf:float = 2):
    result: float = 0
    for integer in vector:
        result = result + integer**(abs_f(inf_norm))
    power: float = abs_f(1/inf_norm)
    result = result**(power) 
    return result

#3
"""A function that takes the argument from a vector stored in a list and returns infinity norm.

Args:
    vector: It is stored as a list.
    inf: It is an integer valued scalar set to default at 2.
    bool: Returns the value as false.

Returns:
    It returns the norm of the input vector. If the boolean value is false, it will return false.
"""
def pick_norm(vector, inf: float =2, get_inf_norm: bool = False):
    result = 1
    same = 1
    for integer in vector:
        same = abs_f(integer)
...
Args:
    vector: It is stored as a list.
    inf: It is an integer valued scalar set to default at 2.
    bool: Returns the value as true.

Returns:
    It returns the norm of the input vector. If the boolean value is true, it will return to infinity norm.
"""
def pick_norm_result(vector, inf: float =2, get_inf_norm: bool = False):
    if (bool == True):
        result = pick_norm(vector)
    else:
        result = pick_norm_result(vector, inf)
    return result  

#5
"""A function that takes arguments from 2 vectors stored in a list.

Args:
    vector_c: It is stored as a list.
    vector_d: It is stored as a list.

Returns:
    The function must handle complex numbers.
"""
def inner_product(vector_c, vector_d):
    result = 0
    for integer in range(len(vector_c)):
        result = result + (complex(vector_c[integer]) * complex(vector_d[integer]))
    return result